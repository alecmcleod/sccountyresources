...
def __remind(event_id,cal_id,iso_date_time,title):
	...
	'''
	try:
		event = models.Event.objects.get(event_id=event_id,calendar_id=cal_id,iso_date_time=iso_date_time)
		...
		if len(events['items']) > 1:
			...
			try:
				...
				#its all day
				date = datetime.strptime(next_event['start']['date'], '%Y-%m-%d').date()
