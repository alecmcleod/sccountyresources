...
class SearchForm(forms.Form):
    ...
    services = forms.ChoiceField(label='I need',
                                 choices=SERVICES_CHOICES,
                                 required=True,
                                 label_suffix="",
                                 widget=forms.Select(attrs={'id': 'services'}))
    locations = forms.ChoiceField(label='near',
                                  choices=LOCATION_CHOICES,
                                  required=False,
                                  label_suffix="",
                                  widget=forms.Select(attrs={'id':'locations'}))
...
class DistanceFilterForm(forms.Form):
    ...
    # First part of search filtering, allows user to select distance to filter by
    DISTANCE_CHOICES = ((1, '1 mile'), (3, '3 miles'), (5, '5 miles'), (10, '10 miles'), (100, 'Santa Cruz County'))
    within_distance = forms.ChoiceField(label='Within:',
                                        choices=DISTANCE_CHOICES,
                                        label_suffix="",
                                        widget=forms.Select(attrs={'id': 'within_distance',
